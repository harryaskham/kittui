...
- PTY children inherit `KITTWM_SOCKET`, `KITTWM_DISPLAY`, `KITTUI_WM_DISPLAY`, and `KITTWM_WINDOW`. The default `~/.config/kittwm/config.yaml` terminal backend is `ghostty`, so `Ctrl-A Enter` launches through the libghostty-backed surface unless overridden; `Ctrl-A t` is reserved for floating-mode toggle. `KITTWM_TERMINAL_CMD` (or `KITTWM_TERMINAL_BINARY` for config-system handoff) selects what command it launches; `terminal.backend`, `libghostty.theme`, `libghostty.background_opacity`, and `libghostty.kitty_graphics` in config control renderer defaults, with env `KITTWM_TERMINAL_BACKEND` / `KITTWM_TERMINAL_APP` still available as overrides.
