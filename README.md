...
- `--interactive` opens a raw-mode rich pager for file inputs; use `j/k`, arrow
  keys, PageUp/PageDown, Home/End, `g/G`, `h`/`?` for in-pager help, `r` to
  reload the file from disk, and `q`. The footer shows the source path, current
  offset/max offset, viewport size, and total rendered rows; reloads also report
  an in-pager status message for both success and transient file errors.
