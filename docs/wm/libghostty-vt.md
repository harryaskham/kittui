...
`kittui-ghostty --kittwm-proof-command COMMAND --out proof.png --cols 100 --rows 28` is the preferred agent proof path for kittwm screenshots. It runs the command in a PTY with kittwm-friendly terminal-renderer environment (`KITTWM_NATIVE_RENDERER=terminal`, terminal chrome, no startup terminal) and renders the resulting VT state to a PNG. When `--pty-input` is supplied to exit a long-running TUI after a delay, the proof path preserves the live PTY bytes captured just before that input so alternate-screen teardown does not replace the screenshot with the restored shell. Use this for caco file image proofs so artifacts show the actual rendered kittwm feature/output instead of text-note PNGs.
