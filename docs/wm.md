...
`kittwm doctor` reports the active graphics transport diagnostics, including selected transport, compression mode, tmux/remote classification, explicit override source, and any conservative fallback reason. The same diagnostic model is available to library callers as `kittui::TransportDiagnostics`.
