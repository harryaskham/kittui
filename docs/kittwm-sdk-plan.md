...
- Extend the new `SurfaceId`, `SurfaceFrame`, `SurfaceMetadata`, `SurfaceCapabilities`, and `NativeSurface` model into the remaining native adapters, including the extended capability flags for exact bytes, focus notifications, and the `take_surface_events` drain hook for terminal side-effect events.
